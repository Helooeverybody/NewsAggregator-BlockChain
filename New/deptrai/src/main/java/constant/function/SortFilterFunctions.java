...
public class SortFilterFunctions {
    public static List<SearchResult> sortByDateAsc(List<SearchResult> searchResults) {
    ...
    public static List<SearchResult> sortByDateDesc(List<SearchResult> searchResults) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MMM dd, yyyy", Locale.ENGLISH);
        return searchResults.stream()
    ...
    public static List<SearchResult> filterByYear(List<SearchResult> searchResults, int year) {
        return searchResults.stream()
                .filter(result -> {
    ...
    public static List<SearchResult> filterByType(List<SearchResult> searchResults, String type) {
        System.out.println("The filter by type has been called");
        return searchResults.stream()
                .filter(result -> result.getType().equalsIgnoreCase(type))
                .collect(Collectors.toList());
    }
    public static List<SearchResult> function(List<SearchResult> searchResults, Sort sort, FilterType filterType, FilterYear filterYear){
        switch (sort){
            case Sort.BY_NEWEST -> {
